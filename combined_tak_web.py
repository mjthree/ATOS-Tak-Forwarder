...
FORWARD_CFG_FILE="forwarding_config.json"
TAK_CFG_FILE="tak_server_config.json"
TEMPLATES_FILE="templates.json"
forwarding_config={"forward_all":False,"tags":{}}
templates_data={}
UNKNOWN_COORD = 9999999.0

# Configuration helpers
def load_forwarding_config():
    global forwarding_config, forward_all
    if os.path.exists(FORWARD_CFG_FILE):
        with open(FORWARD_CFG_FILE,"r") as f:
            forwarding_config=json.load(f)
    forward_all=forwarding_config.get("forward_all", False)

def save_forwarding_config():
    with open(FORWARD_CFG_FILE,"w") as f:
        json.dump(forwarding_config,f,indent=2)

def load_tak_server_config():
    global tak_server_config
    if os.path.exists(TAK_CFG_FILE):
        with open(TAK_CFG_FILE,"r") as f:
            tak_server_config=json.load(f)

def save_tak_server_config():
    with open(TAK_CFG_FILE,"w") as f:
        json.dump(tak_server_config,f,indent=2)
...
def apply_forwarding_config():
    global forward_all
    forward_all=forwarding_config.get("forward_all", False)
    for tid,cfg in forwarding_config.get("tags", {}).items():
        if int(tid) in tag_state:
            tag_state[int(tid)]["forward"]=cfg.get("forward", False)
            tag_state[int(tid)]["callsign"]=cfg.get("callsign", f"Tag_{tid}")
                    ...
                    for packet in packets:
                        try:
                            ...
                            if tag_id not in tag_state:
                                cfg = forwarding_config.get('tags', {}).get(str(tag_id), {})
                                tag_state[tag_id] = {
                                    'forward': cfg.get('forward', False),
                                    'color': 'White',
                                    'callsign': cfg.get('callsign', f'Tag_{tag_id}'),
                                    'last_seen': 0,
                                    'stale': True,
                                    'cots_sent': 0,
                                    'last_good_lat': None,
                            ...
                            if not bad_gps:
                                tag_state[tag_id]['last_good_lat'] = lat
                                tag_state[tag_id]['last_good_lon'] = lon
                                tag_state[tag_id]['last_good_alt'] = altitude
                ...
                try:
                    ...
                    if tag_id not in tag_state or tag_state[tag_id].get('last_seen', 0) == 0:
                        ...
                        tag_state[tag_id] = {
                            ...
                            'last_seen': 0,
                            'cots_sent': 0,
                            'last_good_lat': data.get('latitude') if not data.get('bad_gps') else None,
                            'last_good_lon': data.get('longitude') if not data.get('bad_gps') else None,
                            'last_good_alt': altitude if not data.get('bad_gps') else None
                        }
                        loaded_count += 1
...
def create_cot_xml(tag_id, tag_data):
    ...
    lat_val = tag_data.get('latitude', 0)
    lon_val = tag_data.get('longitude', 0)
    hae_val = tag_data.get('altitude', 0)
    if tag_data.get('bad_gps') and tag_data.get('last_good_lat') is not None:
        lat_val = tag_data.get('last_good_lat')
        lon_val = tag_data.get('last_good_lon')
        hae_val = tag_data.get('last_good_alt', hae_val)
    lat = f"{lat_val:.7f}"
    lon = f"{lon_val:.7f}"
    hae = f"{hae_val:.3f}"
...
def set_tag_forward(tag_id):
    if tag_id in tag_state:
        value = request.json.get('forward', False)
        tag_state[tag_id]['forward'] = value
        forwarding_config.setdefault('tags', {}).setdefault(str(tag_id), {})['forward'] = value
        save_forwarding_config()
...
def set_tag_callsign(tag_id):
    if tag_id in tag_state:
        value = request.json.get('callsign', f'Tag_{tag_id}')
        tag_state[tag_id]['callsign'] = value
        forwarding_config.setdefault('tags', {}).setdefault(str(tag_id), {})['callsign'] = value
        save_forwarding_config()
    return jsonify({'success': True})
...
def forward_all_toggle():
    ...
    if request.method == 'POST':
        ...
        forwarding_config['forward_all'] = forward_all
        save_forwarding_config()
...
def manage_templates():
    ...
    if request.method == 'GET':
        return jsonify(list(templates_data.keys()))
    data = request.json or {}
    action = data.get('action')
    name = data.get('name')
    if action == 'save' and name:
        templates_data[name] = forwarding_config
        save_templates()
        return jsonify({'success': True})
    if action == 'load' and name in templates_data:
        forwarding_config = templates_data[name]
        apply_forwarding_config()
        save_forwarding_config()
        return jsonify({'success': True})
    if action == 'delete' and name in templates_data:
        del templates_data[name]
        save_templates()
        return jsonify({'success': True})
    if action == 'rename' and name in templates_data and 'new_name' in data:
        templates_data[data['new_name']] = templates_data.pop(name)
        save_templates()
        return jsonify({'success': True})
